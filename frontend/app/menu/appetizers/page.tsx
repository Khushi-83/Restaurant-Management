...
const Appetizers = () => {
  const [Appetizers, setAppetizers
  ] = useState<FoodItem[]>([]);
  ...
  useEffect(() => {
    fetch("http://localhost:5000/api/food-items")
      .then((res) => res.json())
      .then((data: FoodItem[]) => {
        const filteredItems = data.filter((item) => item.category === "appetizers");
        setAppetizers
  (filteredItems);
      })
  ...
  return (
    <div className="p-6">
      <h1 className="text-3xl font-bold text-center mb-6">Appetizers
